    ...
    *d_min_dt = std::numeric_limits<CCTK_REAL>::max();
    ...
    {   
        ...
        {   
            ...
            dt1 = fmin(max_dx, dt1);
            if (exp(lnE) / lapse_x > 5.0) { dt1 = 0.0; }
            ...
            dt1 = fmin(max_dx, dt2);
            if (exp(lnE) / lapse_x > 5.0) { dt2 = 0.0; }
            const CCTK_REAL dt3 = get_dt(dt2, dxvecdt, dvvecdtx, xvec, vvec, plo0, phi0, dx, lapse_array, shift_array, metric_array, curv_array, dtfac, 0.5, lapse_x, lev);
            ...
            dt1 = fmin(max_dx, dt3);
            if (exp(lnE) / lapse_x > 5.0) { dt3 = 0.0; }
            ...
            dt1 = fmin(max_dx, dt4);
            if (exp(lnE) / lapse_x > 5.0) { dt4 = 0.0; }
