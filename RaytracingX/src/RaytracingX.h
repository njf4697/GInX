...
#define ASSERT_FINITEP(I, X) if (!std::isfinite(X)) { DEBUGP(I, std::to_string(u[0]) + " " +  std::to_string(u[1]) + " " + std::to_string(u[2])); DEBUGP(I, std::to_string(X) + " is not finite."); assert(std::isfinite(X)); }
