...
void camera_initializer(ParticleContainerClass &pc, const CCTK_REAL *real_params, const CCTK_INT *int_params)
{
  ...
  //fprintf(stderr, "%i %i %i\n", num_pixels, local_particles_size, total_tiles);
  ...
  { 
    ...
    const int local_offset = current_tile * (num_pixels / total_tiles) + std::min(current_tile, num_pixels % total_tiles);
...
#pragma omp parallel for
    ...
    { 
      ...
      if (calculate_kerr_conserved_quantities) {
        ...
        const CCTK_REAL V_down[3] = {arrdata[StructType::vx][local_particle_id], arrdata[StructType::vy][local_particle_id], arrdata[StructType::vz][local_particle_id]};
...
// template <typename StructType, typename ParticleContainerClass>
// void camera_initializer_orthographic(ParticleContainerClass &pc, const CCTK_REAL *real_params, const CCTK_INT *int_params)
// {
//   CCTK_INFO("Initializing particles using the RaytracingX::camera_initializer_orthographic");
...
//    * e3 is parallel to the 'right' direction in the camera's reference frame.
//    * 
//    * width is the horizontal field of view of the camera in radians.
//    * height is the verical field of view of the camera in radians.
//    * lapse is the lapse from the 3+1 spacetime decomposition interpolated at the camera's position.
//    * num_pixels_width and num_pixels_height are the number of pixels of the image horizontally and vertically respectfully.
//    * camera_pos are the x, y, and z coordinates of the camera.
//    */
...
//   e3[2] = real_params[24];
//   e3[3] = real_params[25];
//   CCTK_REAL width = real_params[26];
//   CCTK_REAL height = real_params[27];
...
//       ptd.pos(0, local_particle_id) = camera_pos[0] + a_adj*e3[1] - b_adj*e2[1];
//       ptd.pos(1, local_particle_id) = camera_pos[1] + a_adj*e3[2] - b_adj*e2[2];
//       ptd.pos(2, local_particle_id) = camera_pos[2] + a_adj*e3[3] - b_adj*e2[3];
//       CCTK_REAL A = 1 / lapse * chi[0];
