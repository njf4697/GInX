...
#define ASSERT_FINITE1(X, Y) for (int ii = 0; ii < Y; ii++) { assert(std::isfinite(X[ii])); }
#define ASSERT_FINITE2(X, Y, Z) for (int jj = 0; jj < Z; jj++) { for (int ii = 0; ii < Y; ii++) { assert(std::isfinite(X[jj][ii])); } }
